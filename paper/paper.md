...
To run WEPP at watershed scale, a digital elevation model must be transformed into model-ready representations of channels, hillslopes, outlets, slopes, watershed masks, and channel-network metadata. `weppcloud-wbt` provides this preprocessing layer in a modern Rust geospatial framework while preserving the useful conceptual structure of the TOPAZ digital landscape analysis system [@garbrecht_martz_2004_topaz_overview_manual; @martz_garbrecht_1992_drainage_network_dem; @martz_garbrecht_1993_automated_extraction_dem]. The software adds TOPAZ-style hillslope and channel identifiers, outlet discovery, stream-junction diagnostics, stream-network pruning, flow-vector slope calculation, raster clipping, GeoJSON pour-point watershed support, road-embankment conditioning, basin hierarchy export, improved runtime error propagation, Python wrapper enhancements, and limited read-only VRT support. These tools are orchestrated in WEPPcloud through the `WhiteboxToolsTopazEmulator` adapter, which exposes the WhiteboxTools-based preprocessing pipeline to published WEPPcloud workflows.
...
`weppcloud-wbt` is the terrain-processing and watershed-topology engine used by WEPPcloud to prepare model-ready watershed inputs for WEPP simulations [@lew_etal_2022_weppcloud_part_i; @lew_2026_wepppy]. WEPPcloud is not an experimental prototype: it is a published online hydrologic and erosion TRL9 modeling platform that automatically assembles WEPP inputs from public topography, soils, land-cover, and climate datasets and allows users to run watershed-scale simulations through a web browser [@lew_etal_2022_weppcloud_part_i]. A companion Journal of Hydrology study used WEPPcloud to model 28 forested watersheds across the western United States and compared simulated streamflow, sediment, and phosphorus against observations with minimal or no calibration [@dobre_etal_2022_weppcloud_part_ii]. The associated open data publication archived full WEPPcloud model runs, including raw and processed input/output files, tables, shapefiles, and reproducible watershed configurations [@dobre_etal_2022_weppcloud_datasets].

The research contribution of `weppcloud-wbt` is therefore not merely that it automates GIS preprocessing. It provides the reproducible terrain abstraction layer that allows a published WEPPcloud workflow to move from user-selected outlets and contemporary geospatial rasters to the channel, hillslope, slope, watershed-boundary, and network-table products consumed by WEPP. These steps are scientifically consequential because errors in outlet placement, stream pruning, channel topology, hillslope assignment, flow-vector slope, or watershed clipping change the spatial structure of the hydrologic model before any erosion or runoff equations are evaluated.

The software also makes TOPAZ-style parameterization inspectable and extensible for current research workflows. Legacy TOPAZ concepts remain valuable, but the original workflow is difficult to adapt to cloud execution, GeoTIFF/GeoJSON inputs, VRT-backed rasters, road-conditioned DEMs, and WEPP-specific channel-network constraints. `weppcloud-wbt` implements these adaptations in a compiled, testable, command-line and Python-accessible framework. The repository includes regression tests, synthetic fixtures, tool specifications, reviewer-facing evidence matrices, and WEPPcloud integration points that allow reviewers and future researchers to evaluate the preprocessing behavior independently of the WEPPcloud web interface.
