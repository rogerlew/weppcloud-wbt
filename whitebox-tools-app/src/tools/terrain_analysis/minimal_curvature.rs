...
/// This tool calculates the minimal curvature from a digital elevation model (DEM). Minimal curvature
/// is the curvature of a principal section with the lowest value of curvature at a given point of the
/// topographic surface (Florinsky, 2017). The values of this curvature are unbounded, and positive
/// values correspond to hills while negative values are indicative of valley positions (Florinsky, 2016).
/// Minimal curvature is measured in units of m<sup>-1</sup>.
///
/// ![](../../doc_img/MinimalCurvature.png)
///
...
impl MinimalCurvature {
    pub fn new() -> MinimalCurvature {
        // public constructor
        let name = "MinimalCurvature".to_string();
...
impl WhiteboxTool for MinimalCurvature {
                    ...
                    for row in (0..rows).filter(|r| r % num_procs == tid) {
                        ...
                        for col in 0..columns {
                            ...
                            if z12 != nodata {
                                ...
                                */
                                mean_curv = -((1. + q * q) * r - 2. * p * q * s + (1. + p * p) * t)
                                    / (2. * ((1. + p * p + q * q).powi(3)).sqrt());
                                gaussian_curv = (r * t - s * s) / (1. + p * p + q * q).powi(2);
                                minimal_curv =
                                    mean_curv - (mean_curv * mean_curv - gaussian_curv).sqrt();

                                if log_transform {
                                    // Based on Florinsky (2016) pg. 244 eq. 8.1
                                    minimal_curv = minimal_curv.signum()
                                        * (1. + log_multiplier * minimal_curv.abs()).ln();
                                }

                                data[col as usize] = minimal_curv;
                            }
                        }
