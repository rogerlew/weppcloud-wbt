...
/// This tool calculates the total curvature from a digital elevation model (DEM). Total curvature
/// measures the curvature of the topographic surface rather than the curvature of a line across the
/// surface in some direction (Wilson, 2018). Total curvature can be positive or negative, with zero
/// curvature indicating that the surface is either flat or the convexity in one direction is balanced
/// by the concavity in another direction, as would occur at a saddle point. Total curvature is measured
/// in units of m<sup>-1</sup>.
///
/// ![](../../doc_img/TotalCurvature.png)
///
...
impl TotalCurvature {
    pub fn new() -> TotalCurvature {
        // public constructor
        let name = "TotalCurvature".to_string();
        let toolbox = "Geomorphometric Analysis".to_string();
        let description = "Calculates a total curvature raster from an input DEM.".to_string();
...
impl WhiteboxTool for TotalCurvature {
                        ...
                        for col in 0..columns {
                            ...
                            if z12 != nodata {
                                ...
                                */
                                ...
                                /*
                                The following equation has been taken from Wilson (2018) Environmental Applications
                                of Digital Terrain Modelling, Chapter 3, pg. 66 eq. 3.15.
                                */
                                total_curv = r * r + 2. * s * s + t * t;

                                if log_transform {
                                    // Based on Florinsky (2016) pg. 244 eq. 8.1
                                    total_curv = total_curv.signum()
                                        * (1. + log_multiplier * total_curv.abs()).ln();
                                }

                                data[col as usize] = total_curv;
                            }
                        }
