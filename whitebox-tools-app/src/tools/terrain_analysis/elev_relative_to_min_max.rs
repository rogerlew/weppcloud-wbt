...
impl ElevRelativeToMinMax {
    pub fn new() -> ElevRelativeToMinMax {
        // public constructor
        let name = "ElevRelativeToMinMax".to_string();
        let toolbox = "Geomorphometric Analysis".to_string();
        let description = "Calculates the elevation of a location relative to the minimum and maximum elevations in a DEM.".to_string();
...
impl WhiteboxTool for ElevRelativeToMinMax {
    ...
    fn run<'a>(
        ...
        let range = input.configs.maximum - min_val;
            ...
            thread::spawn(move || {
                ...
                for row in (0..rows).filter(|r| r % num_procs == tid) {
                    ...
                    for col in 0..columns {
                        z = input[(row, col)];
                        if z != nodata {
                            data[col as usize] = (z - min_val) / range * 100f64;
                        }
                    }
