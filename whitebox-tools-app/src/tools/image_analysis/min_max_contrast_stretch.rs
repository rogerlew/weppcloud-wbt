...
///
/// # See Also
/// `PiecewiseContrastStretch`, `GaussianContrastStretch`, `HistogramEqualization`, `PercentageContrastStretch`, `SigmoidalContrastStretch`,
...
impl MinMaxContrastStretch {
    pub fn new() -> MinMaxContrastStretch {
        // public constructor
        ...
        parameters.push(ToolParameter {
            name: "Lower Tail Clip Value".to_owned(),
            flags: vec!["--min_val".to_owned()],
            description: "Lower tail clip value.".to_owned(),
        ...
        parameters.push(ToolParameter {
            name: "Upper Tail Clip Value".to_owned(),
            flags: vec!["--max_val".to_owned()],
            description: "Upper tail clip value.".to_owned(),
            parameter_type: ParameterType::Float,
            default_value: None,
            optional: false,
        });
        ...
        let usage = format!(">>.*{0} -r={1} -v --wd=\"*path*to*data*\" -i=input.tif -o=output.tif --min_val=45.0 --max_val=200.0 --num_tones=1024", short_exe, name).replace("*", &sep);
