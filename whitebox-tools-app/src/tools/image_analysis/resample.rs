    ...
    fn run<'a>(
        ...
        for i in 0..args.len() {
            ...
            if flag_val == "-i" || flag_val == "-inputs" {
                ...
                if cell_size > 0f64 {
                    ...
                    panic!(
                        "Error, when specified, the cell_size parameter must be larger than 0.0."
                    );
                }
        ...
        for i in 0..num_files {
            ...
            if !value.trim().is_empty() {
                ...
                inputs.push(
                    Raster::new(&input_file, "r")
                        .expect(&format!("Error reading image file {}", input_file)),
                );
                num_images += 1;
                nodata_vals.push(inputs[i].configs.nodata);
                if inputs[num_images - 1].configs.west < min_x {
                    min_x = inputs[num_images - 1].configs.west;
                }
                if inputs[num_images - 1].configs.south < min_y {
                    min_y = inputs[num_images - 1].configs.south;
                }
                if inputs[num_images - 1].configs.east > max_x {
                    max_x = inputs[num_images - 1].configs.east;
                }
                if inputs[num_images - 1].configs.north > max_y {
                    max_y = inputs[num_images - 1].configs.north;
                }
            } else {
                return Err(Error::new(ErrorKind::InvalidInput,
