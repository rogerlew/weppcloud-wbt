...
impl TopologicalStreamOrder {
    pub fn new() -> TopologicalStreamOrder {
        // public constructor
        let name = "TopologicalStreamOrder".to_string();
        let toolbox = "Stream Network Analysis".to_string();
        let description =
            "Assigns each link in a stream network its topological order.".to_string();
...
impl WhiteboxTool for TopologicalStreamOrder {
    ...
    fn run<'a>(
        ...
        // Find outlet cells and add them to stack
        ...
        while !stack.is_empty() {
            ...
            // how many inflowing neighbours are there?
            count = 0i8;
            for i in 0..8 {
                row_n = row + d_y[i];
                col_n = col + d_x[i];
            ...
            if count > 0 {
                // add inflowing neighbours to the stack and assign their order value
                for i in 0..8 {
                    row_n = row + d_y[i];
                    col_n = col + d_x[i];
                    if streams[(row_n, col_n)] > 0.0 && pntr[(row_n, col_n)] == inflowing_vals[i] {
