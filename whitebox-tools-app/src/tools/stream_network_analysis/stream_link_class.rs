...
impl StreamLinkClass {
    pub fn new() -> StreamLinkClass {
        // public constructor
        let name = "StreamLinkClass".to_string();
        let toolbox = "Stream Network Analysis".to_string();
        let description =
            "Identifies the exterior/interior links and nodes in a stream network.".to_string();
        ...
        while !stack.is_empty() {
            ...
            if dir > 0 {
                ...
                if num_inflowing[(row_n, col_n)] > 1 {
                    ...
                    // i.e. it hasn't already been assigned a value like at a confluence
                    if val == 3f64 || val == 1f64 {
                        output[(row_n, col_n)] = 1f64;
                    } else {
                        //if val == 4f64 || val == 2f64 {
