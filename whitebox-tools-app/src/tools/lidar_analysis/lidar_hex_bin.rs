...
use whitebox_common::structures::{
    BoundingBox, DistanceMetric, FixedRadiusSearch2D, Point2D, Point3D,
};
use whitebox_lidar::*;
        ...
        if orientation == "h" {
            ...
            for i in 0..num_points as usize {
                ...
                if ret.len() > 0 {
                    ...
                    if (pd.intensity as i32) < min_i[hex_index] {
                        min_i[hex_index] = pd.intensity as i32;
                    }
                    if (pd.intensity as i32) > max_i[hex_index] {
                        max_i[hex_index] = pd.intensity as i32;
                    }
                }
                if verbose {
