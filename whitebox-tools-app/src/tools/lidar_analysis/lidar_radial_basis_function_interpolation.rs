...
use std::thread;
use whitebox_common::structures::{Basis, BoundingBox, RadialBasisFunction};
use whitebox_lidar::*;
use whitebox_raster::*;
                                                ...
                                                || (pd.is_early_return() & early_returns)
                                            {
                                                if include_class_vals[pd.classification() as usize]
                                                {
                                                        ...
                                                        && p.z <= max_z
                                                    {
                                                        ...
                                                        z_values.push(DVector::from_vec(vec![pd
                                                            .classification()
                                                            as f64]));
                                                    }
                                                }
