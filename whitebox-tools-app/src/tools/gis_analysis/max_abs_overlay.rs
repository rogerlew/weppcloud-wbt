...
impl MaxAbsoluteOverlay {
    pub fn new() -> MaxAbsoluteOverlay {
        // public constructor
        let name = "MaxAbsoluteOverlay".to_string();
        let toolbox = "GIS Analysis/Overlay Tools".to_string();
        let description = "Evaluates the maximum absolute value for each grid cell from a stack of input rasters.".to_string();
...
impl WhiteboxTool for MaxAbsoluteOverlay {
                    ...
                    for col in 0..columns {
                        ...
                        if in_val != in_nodata {
                            ...
                            if out_val != out_nodata {
                                if in_val.abs() > out_val {
                                    output[(row, col)] = in_val.abs();
                                }
                            } else {
                                output[(row, col)] = in_val.abs();
                            }
