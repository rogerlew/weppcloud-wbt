...
impl EqualTo {
    pub fn new() -> EqualTo {
        // public constructor
        let name = "EqualTo".to_string();
        let toolbox = "Math and Stats Tools".to_string();
        let description = "Performs a equal-to comparison operation on two rasters or a raster and a constant value.".to_string();
...
impl WhiteboxTool for EqualTo {
    ...
    fn run<'a>(
        ...
        if input1_is_constant && input2_is_constant {
            ...
            println!("{}", equal_to(input1_constant, input2_constant));
            return Ok(());
                ...
                thread::spawn(move || {
                    ...
                    for row in (0..rows).filter(|r| r % num_procs == tid) {
                        ...
                        for col in 0..columns {
                            z2 = in2[(row, col)];
                            if z2 != nodata2 {
                                data[col as usize] = equal_to(input1_constant, z2);
                            } else {
                ...
                thread::spawn(move || {
                    ...
                    for row in (0..rows).filter(|r| r % num_procs == tid) {
                        ...
                        for col in 0..columns {
                            z1 = in1[(row, col)];
                            if z1 != nodata1 {
                                data[col as usize] = equal_to(z1, input2_constant);
                            } else {
                    ...
                    for row in (0..rows).filter(|r| r % num_procs == tid) {
                        ...
                        for col in 0..columns {
                            ...
                            if z1 != nodata1 && z2 != nodata2 {
                                data[col as usize] = equal_to(z1, z2);
                            } else {
                                data[col as usize] = nodata1;
                            }
                        }
...
fn equal_to(val1: f64, val2: f64) -> f64 {
    if val1 == val2 {
        return 1.0;
    }
    0.0
