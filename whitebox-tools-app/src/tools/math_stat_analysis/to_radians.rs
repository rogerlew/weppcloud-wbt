...
impl ToRadians {
    /// public constructor
    pub fn new() -> ToRadians {
        let name = "ToRadians".to_string();
        let toolbox = "Math and Stats Tools".to_string();
        let description = "Converts a raster from degrees to radians.".to_string();
...
impl WhiteboxTool for ToRadians {
            ...
            thread::spawn(move || {
                ...
                for row in (0..rows).filter(|r| r % num_procs == tid) {
                    ...
                    for col in 0..columns {
                        z = input[(row, col)];
                        if z != nodata {
                            data[col as usize] = z.to_radians();
                        }
                    }
