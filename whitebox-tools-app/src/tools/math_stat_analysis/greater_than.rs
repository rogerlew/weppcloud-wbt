...
///
/// Use the `--incl_equals` flag to perform a greater-than-equal-to (>=) operation instead of the default greater-than (>)
/// operator.
///
/// # See Also
...
impl GreaterThan {
    pub fn new() -> GreaterThan {
        ...
        parameters.push(ToolParameter {
            name: "Perform a greater-than-OR-EQUAL-TO operation?".to_owned(),
            flags: vec!["--incl_equals".to_owned()],
            description: "Perform a greater-than-or-equal-to operation.".to_owned(),
...
impl WhiteboxTool for GreaterThan {
    ...
    fn run<'a>(
        ...
        if input1_is_constant && input2_is_constant {
            ...
            println!(
                "{}",
                greater_than(input1_constant, input2_constant, equal_to)
            );
            return Ok(());
                ...
                thread::spawn(move || {
                    ...
                    for row in (0..rows).filter(|r| r % num_procs == tid) {
                        ...
                        for col in 0..columns {
                            z2 = in2[(row, col)];
                            if z2 != nodata2 {
                                data[col as usize] = greater_than(input1_constant, z2, equal_to);
                            } else {
                ...
                thread::spawn(move || {
                    ...
                    for row in (0..rows).filter(|r| r % num_procs == tid) {
                        ...
                        for col in 0..columns {
                            z1 = in1[(row, col)];
                            if z1 != nodata1 {
                                data[col as usize] = greater_than(z1, input2_constant, equal_to);
                            } else {
                    ...
                    for row in (0..rows).filter(|r| r % num_procs == tid) {
                        ...
                        for col in 0..columns {
                            ...
                            if z1 != nodata1 && z2 != nodata2 {
                                data[col as usize] = greater_than(z1, z2, equal_to);
                            } else {
                                data[col as usize] = nodata1;
                            }
                        }
...
fn greater_than(val1: f64, val2: f64, equal_to: bool) -> f64 {
    if !equal_to {
        if val1 > val2 {
            return 1.0;
        }
