...
/// raster (`--output`) will be assigned the maximum value of each of the input values. **NoData** values
...
impl Max {
    ...
    pub fn new() -> Max {
        ...
        let description =
            "Performs a MAX operation on two rasters or a raster and a constant value.".to_string();
...
impl WhiteboxTool for Max {
    ...
    fn run<'a>(
        ...
        if input1_is_constant && input2_is_constant {
            ...
            println!("{}", input1_constant.max(input2_constant));
            return Ok(());
                ...
                thread::spawn(move || {
                    ...
                    for row in (0..rows).filter(|r| r % num_procs == tid) {
                        ...
                        for col in 0..columns {
                            z2 = in2[(row, col)];
                            if z2 != nodata2 {
                                data[col as usize] = input1_constant.max(z2);
                            } else {
                ...
                thread::spawn(move || {
                    ...
                    for row in (0..rows).filter(|r| r % num_procs == tid) {
                        ...
                        for col in 0..columns {
                            z1 = in1[(row, col)];
                            if z1 != nodata1 {
                                data[col as usize] = z1.max(input2_constant);
                            } else {
