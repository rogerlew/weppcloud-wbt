...
use whitebox_common::structures::{LineSegment, Point2D};
...
fn run(args: &Vec<String>) -> Result<(), std::io::Error> {
    ...
    (0..inputs.len()).into_par_iter().for_each(|k| {
        ...
        {
            let mut input_num_features = input_num_features
                .lock()
                .expect("Error unlocking input_num_features.");
        ...
        for record_num in 0..input.num_records {
            ...
            for part in 0..record.num_parts as usize {
                ...
                polylines.push(Polyline::new(&record.points[part_start..=part_end], fid));
        ...
        for fid in 0..num_polylines {
            ...
            if !marked_for_deletion[fid] {
                let is_joined_at_start =
                    connections[fid][0] < num_polylines && connections[fid][1] == num_polylines;
                let mut is_joined_at_end =
                    connections[fid][0] == num_polylines && connections[fid][1] < num_polylines;
                // if fid == 21414 || fid == 16471 || fid == 3703 || fid == 3683 {
                ...
                if is_joined_at_start || is_joined_at_end {
                    ...
                    if is_joined_at_end {
                        pl.vertices
                            .extend_from_slice(&polylines[fid].vertices.clone());
                    } else {
                    ...
                    loop {
                        ...
                        current_fid = if connections[current_fid][0] < num_polylines
                            && !marked_for_deletion[connections[current_fid][0]]
                        {
                            connections[current_fid][0]
                        } else if connections[current_fid][1] < num_polylines
                            && !marked_for_deletion[connections[current_fid][1]]
                        {
                        ...
                        if pl
                            .get_last_node()
                            .distance(&polylines[current_fid].get_first_node())
                            <= precision
                        {
                            is_joined_at_end = true;
                        }
                        ...
                        if is_joined_at_end {
                            pl.vertices
                                .extend_from_slice(&polylines[current_fid].vertices.clone());
                        } else {
        ...
        for fid in 0..num_polylines {
            ...
            for i in 1..polylines[fid].vertices.len() {
                line_segments.push(GeomWithData::new(
                    Line::new(
                        [
                            polylines[fid].vertices[i - 1].x,
                            polylines[fid].vertices[i - 1].y,
                        ],
            ...
            p1 = polylines[fid].get_first_node();
            end_nodes.push(Location::new([p1.x, p1.y], fid));
        ...
        let mut line_seg2: LineSegment =
            LineSegment::new(Point2D::new(0f64, 0f64), Point2D::new(0f64, 0f64));
        ...
        for poly_id in 0..polylines.len() {
            ...
            if (min_dist.is_finite() && min_dist > precision)
                || (min_dist <= precision && num_neighbours == 0)
            {
        ...
        for fid in 0..polylines2.len() {
            percent_reverse[fid] =
                100.0 * percent_reverse[fid] / (polylines2[fid].len() - 1) as f64;
        ...
        if reverse_backward_arcs {
            ...
            for fid in 0..polylines2.len() {
                ...
                vertices.push(Location2::new([p1.x, p1.y], (fid, 1)));

                p1 = polylines2[fid].get_last_node();
                vertices.push(Location2::new([p1.x, p1.y], (fid, 0)));
            ...
            let mut output = Shapefile::initialize_using_file(
                &output_file.replace(".shp", "_reversed_arcs.shp"),
                &input,
                ShapeType::PolyLine,
                false,
            )
            ...
            for fid in 0..polylines2.len() {
                if reverse[fid] {
                    ...
                    output
                        .attributes
                        .add_record(vec![FieldData::Int((fid + 1) as i32)], false);
                }
            }
        ...
        for poly_id in 0..polylines2.len() {
            ...
            output
                .attributes
                .add_record(vec![FieldData::Int((poly_id + 1) as i32)], false);
        ...
        {
            let mut output_num_features = output_num_features
                .lock()
                .expect("Error unlocking output_num_features.");
    ...
    let input_num_features = input_num_features
        .lock()
        .expect("Error locking output_num_features.");
    println!("Num. input line features: {}", *input_num_features);

    let output_num_features = output_num_features
        .lock()
        .expect("Error locking output_num_features.");
...
impl Polyline {
    ...
    fn split(&mut self) -> Vec<Self> {
        ...
        let mut dist: f64;
        for split in 0..self.split_points.len() {
            dist = segment_distances[self.split_points[split].0]
                + self[self.split_points[split].0].distance(&self.split_points[split].1);
        ...
        if self.split_points.len() > 0 {
            ...
            for node in 0..self.len() {
                if current_split == self.split_points.len()
                    || node < self.split_points[current_split].0
                {
                    line.push(self[node]);
                } else {
                    line.push(self[node]);
                    ...
                    loop {
                        ...
                        if current_split < self.split_points.len()
                            && former_node == self.split_points[current_split].0
                        {
                            line.push(self.split_points[current_split].1);
                            ret.push(line);
                            line = Polyline::new_empty(self.id);
