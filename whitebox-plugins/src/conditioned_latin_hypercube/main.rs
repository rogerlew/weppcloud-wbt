...
fn run(args: &Vec<String>) -> Result<(), std::io::Error> {
    ...
    let seed: u64 = if rng_seed < 0 {
        // no seed specified, random seed
    ...
    let k_pool: Vec<usize> = k_pool
        .into_iter()
        .map(|k| ((k / k_pool_sum) * max_iter as f64).ceil() as usize)
        .collect();
    let reservoir_size = (num_files * num_samples) + k_pool.iter().fold(0, |a, v| a + v);
    ...
    for k in 0..num_files {
            ...
            for c in 0..columns {
                ...
                if val != nodata {
                    ...
                    if !is_cont {
                        ...
                        let counter = class_histo.entry(val as isize).or_insert(0);
        ...
        let mut valid_indices: Vec<usize> = valid_indices
            .into_iter()
            .filter(|i| *i != nodata)
            .map(|i| i as usize)
            .collect();
        ...
        */
        ...
        for _ in 0..(num_samples + k_pool[k]) {
            ...
            repeater += 1;
            if repeater == valid_indices.len() {
                // next iteration will panic
    ...
    for a in 0..num_files {
        ...
        for b in (a + 1)..num_files {
            ...
            for tid in 0..num_procs {
                ...
                thread::spawn(move || {
                    ...
                    for i in (0..len1.min(len2) as isize).filter(|i| i % num_procs == tid) {
                        z1 = data1[i as usize];
                        z2 = data2[i as usize];
                        d1_totaldev += (z1 - averages[a]) * (z1 - averages[a]);
                        d2_totaldev += (z2 - averages[b]) * (z2 - averages[b]);
                        sum_productdev += (z1 - averages[a]) * (z2 - averages[b]);
                    }
            ...
            cormat[a][b] = t_product_deviations / (d1_total_deviation * d2_total_deviation).sqrt();
    ...
    // perform analysis
    if verbose {
        println!("Staring resampling operation...");
    }
    ...
    for it in 0..max_iter {
        ...
        if obj <= o_thresh {
            // exit with current samples if obj <= o_thresh
            if verbose {
        ...
        if it % anneal_cycle == anneal_cycle - 1 {
            temp *= temp_decay;
        }
        ...
        if rand2 < rs_prob || max <= 1 {
            ...
            //swap random worst strata sample with fresh random sample
            //pick random worst sample by index
            worst_indices = (0..samples.len())
                .filter(|s| samples[*s].k == worst_k && samples[*s].q == worst_q)
                .collect();
    ...
    let filenames: Vec<&str> = input_files
        .iter()
        .map(|p| {
            path::Path::new(p.trim())
                .file_name()
                .unwrap()
                .to_str()
                .unwrap()
    ...
    for s in 0..samples.len() {
        ...
        output.attributes.add_record(
            vec![
                FieldData::Int((s + 1) as i32),
                FieldData::Real(samples[s].v),
                FieldData::Text(filenames[samples[s].k].to_string()),
            ],
            false,
        );
    }
...
fn sample_correlation_matrix(samples: &Vec<Sample>, size: usize) -> Vec<Vec<f64>> {
    ...
    for a in 0..size {
        ...
        let a_mean = a_samples.iter().fold(0f64, |acc, s| acc + s.v) / a_samples.len() as f64;
        let a_tdev = a_samples
            .iter()
        ...
        for b in (a + 1)..size {
            ...
            let b_mean = b_samples.iter().fold(0f64, |acc, s| acc + s.v) / b_samples.len() as f64;
            let b_tdev = b_samples
                .iter()
...
fn get_eta(samples: &Vec<Sample>, quantiles: &Vec<Vec<f64>>) -> Vec<Vec<usize>> {
    let mut counts_matrix: Vec<Vec<usize>> =
        quantiles.iter().map(|q| vec![0usize; q.len()]).collect(); // set a copy of quantiles to 0;
    for s in 0..samples.len() {
...
fn get_flattened_valid_data(data: Raster) -> Vec<f64> {
    ...
    let mut output: Vec<f64> = Vec::with_capacity(data.configs.rows * data.configs.columns);
    let mut z: f64;
